...
public class MultipartRequestHandler {
	...
	private static final String ARCHIVE_DIR = "archives/jugando/imagenes/";
	private static final String UPLOAD_DIRECTORY = "C:/opt/apache-tomcat-7.0.26/webapps/ROOT/" + ARCHIVE_DIR;
	...
	public static List<FileMeta> uploadByApacheFileUpload(HttpServletRequest request)
			throws IOException, ServletException {
			...
			try {
				...
				for (FileItem item : items) {
					...
					if (item.isFormField()) {
						...
						temp.setLength(item.getSize() / 1024 + "Kb");
						temp.setUrl("../" + ARCHIVE_DIR + temp.getFileName());
	...
	public static FilesMeta uploadByApacheFileUploads(HttpServletRequest request)
			throws IOException, ServletException {

		FilesMeta files = new FilesMeta();
		List<FileMeta> filesMeta = new LinkedList<FileMeta>();
		...
		if (isMultipart) {
			...
			try {
				...
				files.setFiles(filesMeta);
				// 2.8 Set "twitter" parameter
				for (FileMeta fm : filesMeta) {
					fm.setTwitter(twitter);
				}
	...
	public static UploadesFiles uploadByOtherFileUpload(HttpServletRequest request)
			throws IOException, ServletException {
		
		UploadesFiles uFiles = new UploadesFiles();
		...
		if (isMultipart) {
			...
			try {
				...
				for (FileItem file : files) {
					...
					u.setUrl("../" +ARCHIVE_DIR+file.getName());
					u.setThumbnailUrl("");
					u.setDeleteUrl("../"+"delete");
					u.setDeleteType("DELETE");
					
					try {
						file.write(new File(UPLOAD_DIRECTORY + file.getName()));
					} catch (Exception e) {
						// TODO Auto-generated catch block
						System.out.println(e.getMessage());
					}
